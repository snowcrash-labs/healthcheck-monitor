...
use crate::{
    History,
    cost_schema::{cost_partition as p, cost_source as s},
    error::Error,
};
use diesel::prelude::*;
...
pub fn statuses(config: &Config, filter: &Filter, stored: Vec<SourceStatus>) -> Vec<SourceStatus> {
    config
        .sources
        .iter()
        .filter(|source| filter.provider.is_none_or(|p| p == source.provider))
        .map(|source| {
            ...
            if status.fault.is_none()
                && status.imported_at.is_some_and(|at| {
...
impl History {
    pub async fn cost_covered(
        ...
        if sources.is_empty()
            || sources
                .iter()
                .any(|s| s.fault.is_some() || s.state == "stale")
        {
            return Ok(false);
        }
        ...
        let counts: Vec<(String, i64)> = p::table
            .inner_join(s::table.on(s::cost_source_id.eq(p::cost_partition_source_id)))
            .filter(s::cost_source_name.eq_any(&names))
            .filter(p::cost_partition_day.ge(period.from))
            .filter(p::cost_partition_day.lt(period.to))
            .group_by(s::cost_source_name)
            .select((
                s::cost_source_name,
                diesel::dsl::count(p::cost_partition_id),
            ))
            .load(&mut connection)
            .await?;
        ...
        Ok(names
            .iter()
            .all(|name| counts.iter().any(|(n, count)| n == name && *count == days)))
    }
}
