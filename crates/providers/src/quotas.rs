...
use monitor_integrations::{
    projection::{number, observation, operation, text},
    transport::Error,
};
use std::collections::BTreeMap;
use tokio_util::sync::CancellationToken;
pub fn project(job: &Job, endpoint: &Endpoint, row: &serde_json::Value) -> Vec<Observation> {
    let Some(limit) = number(row, &["/Value"]).filter(|v| *v > 0.0) else {
        return vec![];
    };
    ...
    let region = endpoint
        ...
        .unwrap_or_default();
    let usage = text(row, &["/UsageMetric/MetricNamespace"])
        .zip(text(row, &["/UsageMetric/MetricName"]))
        .map(|(namespace, name)| {
            let dimensions = row
                .pointer("/UsageMetric/MetricDimensions")
...
pub async fn evaluate(
    ...
    for (region, queries) in regions {
        ...
        metrics.target.regions = vec![region];
        metrics.target.metrics = queries;
    ...
    if result
        .observations
        .iter()
        .any(|observation| matches!(observation.data, Data::Quota { usage: None, .. }))
    {
