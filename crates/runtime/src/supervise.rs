...
pub async fn monitor(
    ...
    observer.update(&state.snapshot, &effective, &[]);
    observer.heartbeat(chrono::Utc::now(), true);
    ...
    loop {
        tokio::select! {
            _ = stop.cancelled(), if cleanup_deadline.is_none() => { cancelled = true; cleanup_deadline = Some(tokio::time::Instant::now() + std::time::Duration::from_secs(4)); },
            ...
            _ = history.tick(), if matches!(mode, Mode::Watch { .. }) && cleanup_deadline.is_none() => {
                ...
                observer.update(&state.snapshot, &effective, &expired);
            ...
            result = results.recv() => {
                ...
                observer.update(&state.snapshot, &effective, &new);
    ...
    observer.update(&state.snapshot, &effective, &[]);
    observer.heartbeat(chrono::Utc::now(), false);
    if cancelled {
        return Ok(130);
    }
    Ok(match mode {
