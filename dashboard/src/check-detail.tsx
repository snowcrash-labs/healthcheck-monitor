...
import { checkCatalog, checkState, checkStateText, interval, targetPath } from "./check-catalog";
...
export default function CheckDetailPage() {
  ...
  const tab = () => search.tab === "runs" ? "runs" : "operations";
  ...
  return <><a class="back-link" href={query("/checks", { target: target() })}>← Checks</a>
    ...
    <Show when={check.data()} fallback={<Empty title={check.error() ? "Check unavailable" : "Loading check"} />}>{(row) => <>
      ...
      <nav class="tabs" aria-label="Check sections"><button class="tab" aria-current={tab() === "operations" ? "page" : undefined} onClick={() => setSearch({ tab: undefined })}>Operations</button><button class="tab" aria-current={tab() === "runs" ? "page" : undefined} onClick={() => setSearch({ tab: "runs" })}>Run history</button></nav>
      <Show when={tab() === "operations"}><section class="panel"><div class="panel-heading"><h2>Collection operations</h2><label>Search operations<input type="search" value={q()} onInput={(e) => setSearch({ q: e.currentTarget.value || undefined })} /></label></div>
