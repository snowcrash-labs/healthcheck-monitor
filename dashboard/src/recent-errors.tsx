...
export default function RecentErrors() {
  ...
  const result = useQuery(() => query("/api/v1/query/diagnostics", { target: dashboard?.target(), lookback_seconds: value("from") || value("to") ? undefined : value("lookback") || "3600", from: value("from"), to: value("to"), q: value("q"), cluster: value("cluster"), namespace: value("namespace"), cursor: value("cursor") }), pageSchema, () => dashboard?.refreshId() ?? 0);
