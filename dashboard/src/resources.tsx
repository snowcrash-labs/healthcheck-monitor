...
export default function ResourcesPage() {
  ...
  return <>
    ...
    <section class="panel" aria-busy={result.loading() ? "true" : "false"}><Show when={result.data()} fallback={<Empty title="Loading resources" />}>{(page) => <>
      <Show when={page().items.length} fallback={<Empty title="No matching resources" detail="Try another target, health state, or search." />}><div class="table-scroll"><table><thead><tr><th>Resource</th><th>Target</th><th>Health</th><th>Observation</th><th>Freshness</th></tr></thead><tbody><For each={page().items}>{(resource) => <tr><td><a class="primary-link resource-name" href={resourcePath(resource.id, dashboard.target())}>{resource.id}</a><span class="cell-detail">{rule(resource.check)}</span></td><td>{resource.target}</td><td><Status health={stale(resource.expires_at, dashboard.now()) ? "unknown" : resource.health} /></td><td><span>{resource.facts[0]?.label ?? "Metadata"}</span><span class="cell-detail">{resource.facts[0]?.value ?? "Observed"}</span></td><td><span class={stale(resource.expires_at, dashboard.now()) ? "text-warning" : ""}>{stale(resource.expires_at, dashboard.now()) ? "Stale" : age(resource.observed_at, dashboard.now())}</span></td></tr>}</For></tbody></table></div></Show>
