...
import { ConsoleLinks, findingTitle } from "./diagnostics";
...
export default function ResourcesPage() {
  ...
  return <>
    ...
    <section class="panel" aria-busy={result.loading() ? "true" : "false"}><Show when={result.data()} fallback={<Empty title="Loading resources" />}>{(page) => <>
      ...
      <Show when={page().items.length} fallback={<Empty title="No matching resources" detail="Try another target, health state, or search." />}><div class="table-scroll"><table><thead><tr><th>Resource</th><th>Target</th><th>Health</th><th>Findings</th><th>Observation</th><th>Freshness</th></tr></thead><tbody><For each={page().items}>{(resource) => <tr data-row-key={resource.id}>
        <td><a class="primary-link resource-name" href={resourcePath(resource.id, dashboard.target())}>{resource.id}</a><span class="cell-detail">{rule(resource.check)}</span><ConsoleLinks links={resource.links} /></td><td>{resource.target}<span class="cell-detail">{[resource.context?.scope, resource.context?.zone ?? resource.context?.region, resource.context?.namespace].filter(Boolean).join(" · ")}</span></td>
        <td><Status health={stale(resource.expires_at, dashboard.now()) ? "unknown" : resource.health} /></td>
        <td class="resource-findings"><For each={resource.findings} fallback={<span class="muted">No active findings</span>}>{(finding) => <div class="resource-finding"><a class={`finding-rule ${finding.severity === "error" ? "text-error" : finding.severity === "warning" ? "text-warning" : "muted"}`} href={resourcePath(resource.id, dashboard.target())}>{findingTitle(finding.rule)}</a><span class="cell-detail">{rule(finding.severity)} · {finding.stale || stale(finding.valid_until, dashboard.now()) ? "Stale evidence" : age(finding.observed_at, dashboard.now())}</span></div>}</For><Show when={resource.finding_count > resource.findings.length}><a class="quiet-link" href={resourcePath(resource.id, dashboard.target())}>+{resource.finding_count - resource.findings.length} more</a></Show></td>
        <td><For each={(resource.findings[0]?.diagnostic?.facts ?? resource.facts).slice(0, resource.finding_count ? 3 : 1)} fallback={<span class="muted">Metadata observed</span>}>{(fact) => <div class="resource-fact"><span>{fact.label}</span><span class="cell-detail">{fact.value}</span></div>}</For></td>
