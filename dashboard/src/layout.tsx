...
function Layout(props: ParentProps) {
  ...
  return <div class="app-shell">
    ...
    <aside class="sidebar">
      ...
      <nav aria-label="Main navigation"><a href={path("/")}><span aria-hidden="true">◫</span>Overview</a><a href={path("/checks")}><span aria-hidden="true">✓</span>Checks</a><a href={path("/findings")}><span aria-hidden="true">◉</span>Findings</a><a href={path("/resources")}><span aria-hidden="true">▦</span>Resources</a><a href={path("/history")}><span aria-hidden="true">◷</span>History</a></nav>
    ...
    <div class="workspace">
      <header class="topbar"><div class="breadcrumb">Operations <span>/</span> Infrastructure</div><div class="topbar-actions"><span class="updated">{age(dashboard.overview()?.captured_at ?? null, dashboard.now())}</span><label class="target-picker">Target<select aria-label="Monitoring target" value={dashboard.target()} onChange={(event) => dashboard.setTarget(event.currentTarget.value)}><option value="" selected={!dashboard.target()}>All targets</option><For each={dashboard.overview()?.targets ?? []}>{(target) => <option value={target.name} selected={target.name === dashboard.target()}>{target.name}</option>}</For></select></label><ThemePicker /><button class="button refresh-button" onClick={dashboard.refresh}>↻ <span>Refresh view</span></button></div></header>
