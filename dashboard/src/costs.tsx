...
export default function CostsPage() {
  ...
  return <>
    ...
    <Show when={result.error()}><Notice error>{result.error()}</Notice><button class="button" onClick={() => setParams({ cursor: undefined, revision: undefined })}>Refresh cost selection</button></Show>
    <Show when={result.data()} fallback={<Empty title="Loading costs" />}>{(view) => <>
      <Show when={view().enabled} fallback={<Empty title="Billing is not connected" detail="Billing imports need an approved reader group and configured source. No estimated or synthetic charges are shown." />}>
        <div class="filters cost-filters">
          ...
          <label>Group by<select value={value("group") || "provider"} onChange={(e) => { set("group", e.currentTarget.value); setParams({ contributor: undefined }); }}><option value="provider">Provider</option><option value="product">Cloud product</option><option value="scope">Project / account / subscription</option><option value="region">Region</option><option value="category">Charge category</option><option value="resource">Resource</option></select></label>
