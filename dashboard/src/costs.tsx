...
import { costView, contributorLabel, difference, hasCredits, money, net, units } from "./cost-schema";
...
export default function CostsPage() {
  ...
  const select = (day: string, key: string) => setParams({ day: day || undefined, contributor: key || undefined, from: value("from") || result.data()?.period.from, to: value("to") || result.data()?.period.to, ...(key === "__other__" ? { q: undefined } : {}), cursor: undefined, revision: undefined });
  return <>
    ...
    <Show when={result.data()} fallback={<Empty title="Loading costs" />}>{(view) => <>
      <Show when={view().enabled} fallback={<Empty title="Billing is not connected" detail="Billing imports need an approved reader group and configured source. No estimated or synthetic charges are shown." />}>
        ...
        <Show when={value("day") || value("contributor") || value("resource")}><div class="active-filters"><span>{[value("day"), value("contributor") ? contributorLabel(value("contributor")) : "", value("resource")].filter(Boolean).join(" · ")}</span><button class="filter-chip" onClick={() => setParams({ day: undefined, contributor: undefined, resource: undefined, cursor: undefined, revision: undefined })}>Clear selection ×</button></div></Show>
        <section class="panel"><div class="cost-headline"><strong>{money(view().total, view().currency)}</strong><span>{difference(view().total, view().previous_total)}</span><Show when={hasCredits(view().credits)}><span class="cost-credits">Credits {money(view().credits, view().currency)} · Net {money(net(view().total, view().credits), view().currency)}</span></Show><small>Spend before credits · Charge dates in UTC · Provisional until provider reconciliation</small></div>
        ...
        <section class="panel"><div class="panel-heading"><h2>Cost breakdown</h2><label>Search contributors<input type="search" value={value("q")} maxlength={128} onInput={(e) => set("q", e.currentTarget.value)} /></label></div><div class="table-scroll"><table><thead><tr><th>Contributor</th><th class="numeric">Spend</th><th class="numeric" title="Promotions, discounts, and committed-use credits applied to this spend">Credits</th><th class="numeric">Share</th><th title="Change from the preceding period of equal length">Change</th><th>Attribution</th></tr></thead><tbody><For each={view().breakdown} keyed={(row) => row.key}>{(row) => <tr><td><button class="text-button" onClick={() => select("", row().key)}>{contributorLabel(row().key)}</button></td><td class="numeric">{money(row().amount, view().currency)}</td><td class="numeric">{hasCredits(row().credits) ? money(row().credits, view().currency) : "None"}</td><td class="numeric">{view().total && units(view().total ?? "0") > 0n && units(row().amount) >= 0n ? `${(Number(units(row().amount) * 1000n / units(view().total ?? "1")) / 10).toFixed(1)}%` : "Not applicable"}</td><td>{difference(row().amount, row().previous, true)}</td><td>{row().key === "v:" || row().key === "" ? "Unallocated" : view().group === "target" ? "Configured mapping" : "Provider reported"}</td></tr>}</For></tbody></table></div><Pagination current={value("cursor")} next={view().next_cursor} total={view().contributor_count} onChange={(cursor) => setParams({ cursor: cursor || undefined, revision: cursor ? view().revision : undefined, from: value("from") || view().period.from, to: value("to") || view().period.to })} /></section>
      </Show>
