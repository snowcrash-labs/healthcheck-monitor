  ...
  return <>
    ...
    <Show when={result.data()} fallback={<Empty title="Loading costs" />}>{(view) => <>
      <Show when={view().enabled} fallback={<Empty title="Billing is not connected" detail="Billing imports need an approved reader group and configured source. No estimated or synthetic charges are shown." />}>
        ...
        <section class="panel"><div class="panel-heading"><h2>Cost breakdown</h2><label>Search contributors<input type="search" value={value("q")} maxlength={128} onInput={(e) => set("q", e.currentTarget.value)} /></label></div><div class="table-scroll"><table><thead><tr><th>Contributor</th><th class="numeric">Billed cost</th><th class="numeric">Share</th><th title="Change from the preceding period of equal length">Change</th><th>Attribution</th></tr></thead><tbody><For each={view().breakdown} keyed={(row) => row.key}>{(row) => <tr><td><button class="text-button" onClick={() => select("", row().key)}>{contributorLabel(row().key)}</button></td><td class="numeric">{money(row().amount, view().currency)}</td><td class="numeric">{view().total && units(view().total ?? "0") > 0n && units(row().amount) >= 0n ? `${(Number(units(row().amount) * 1000n / units(view().total ?? "1")) / 10).toFixed(1)}%` : "Not applicable"}</td><td>{difference(row().amount, row().previous, true)}</td><td>{row().key === "v:" || row().key === "" ? "Unallocated" : view().group === "target" ? "Configured mapping" : "Provider reported"}</td></tr>}</For></tbody></table></div><Pagination current={value("cursor")} next={view().next_cursor} total={view().contributor_count} onChange={(cursor) => setParams({ cursor: cursor || undefined, revision: cursor ? view().revision : undefined, from: value("from") || view().period.from, to: value("to") || view().period.to })} /></section>
      </Show>
