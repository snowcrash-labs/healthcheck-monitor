...
Profiles are editable TOML presets. A check's `enabled = false` removes its scheduled assessment; other selected checks can still collect facts they require. Required prerequisites are selected automatically, and unavailable prerequisites leave dependent coverage incomplete. `--check`, `--target`, and `--resource` narrow the assessment. Prerequisite facts remain visible in reports without adding unrelated health findings. Resource selectors match resource identity substrings and use the same validation in TOML and CLI input. Regional resources outside the selected locations remain inventory-only, and reports show region and resource filters.
...
Kubernetes exec plugins receive noninteractive input and run through the shared bounded process runner; expired credentials refresh before native clients are reused. Legacy GCP command profiles support their standard dot-path token and expiry selectors. Azure caches tokens for four fixed API audiences. AWS retains native profile parsing, role assumption, SSO and workload providers while routing selected credential-process profiles through bounded execution. All configured AWS service reads use official `aws-sdk-rust` clients sharing those credentials and the bounded reqwest transport. No credential files are rewritten.
...
Evidence admission uses conservative byte estimates before pages and provider results accumulate. Each collection receives a share of the configured memory budget based on global concurrency, and queued request bodies count toward their own bound. Large inventories can reach the byte limit before `max_assets`; lower concurrency or raise `memory_bytes` to retain more evidence per collection. Limit exhaustion marks coverage incomplete and cannot establish recovery or removal. Effective configuration copies are also bounded before scheduling.
...
GCP presets retain the latest backlog/age readings, sustained capacity minima, event totals, and sampled latency means. Cloud Run latency is measured in milliseconds; a distribution without samples is missing evidence. Request and push-delivery failures remain separate series, and dead-letter counts are explicit. Eventarc Standard delivery uses its Pub/Sub transport metrics. See the [GCP metric catalog](https://docs.cloud.google.com/monitoring/api/metrics_gcp_p_z) and [Eventarc delivery diagnostics](https://docs.cloud.google.com/eventarc/standard/docs/run/debugging-events-cloud-run).

GitHub workflow and commit history uses `runtime_window`, while configured reference heads are resolved separately. A literal commit reference is verified against the repository before it becomes evidence. Use `repository_refs` or a longer window when older source revisions matter. Open pull requests use smaller pages to stay within response limits. Identity checks execute independently of the inventory cache.
