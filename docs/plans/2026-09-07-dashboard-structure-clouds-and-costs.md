...
Deliver a UX overhaul and interactive billing charts. Replace the current overview/check-card layout, resource lists, and detail structure with concise operational screens. Make it immediately clear which service or resource, in which environment and cloud, during which period, what is wrong, and what evidence supports that conclusion. Readable typography, spacing, hierarchy, navigation, working investigation links, and stable updates are acceptance requirements.

The [dashboard experience specification](2026-09-07-dashboard-experience.md) defines the required screens, competitor references, visual layout proposals, chart interactions, and a checklist against the requested improvements. It is the product-facing entry point to this plan. Implementation is incomplete until the redesigned screens and a chart backed by real billing data are usable; backend ingestion, wording changes, or mock charts alone do not satisfy the task.
...
Review conclusion: implement the navigation/content changes and reported investigation defects first. Cloud coverage and billing are independently deliverable additions. Do not make a service catalog, billing access decision, or complete historical reconstruction a prerequisite for fixing today's dashboard. Start with existing target/resource identity and add service attribution when mappings are known.
...
The [experience specification](2026-09-07-dashboard-experience.md#competitor-review) adds direct review of Datadog's published Cost Explorer and Resource Catalog screenshots, monitor search, Grafana investigation flows, and New Relic's current Hosts UI. Adopt the graph/table/detail pattern and context-preserving navigation. Keep the product an opinionated health and cost console; missing processing-progress telemetry remains visible, and infrastructure health alone cannot establish end-to-end client usability.
...
Validate catalog keys and selectors on atomic configuration reload. Ambiguous primary-service matches remain Unassigned with a configuration diagnostic; they must not duplicate resource or cost totals. Explicit weighted shared-cost allocation is separate from primary ownership. Preserve canonical resource identity, including provider scope and Kubernetes UID, across grouping and navigation. Recreated resources with the same display name retain separate failure episodes.
...
| View | Meaning of time | Source and limitation |
| --- | --- | --- |
| Overview, Resources, Active problems, current Checks | Current published state, labeled Live or Paused at an exact capture time. | Existing current-state APIs. Do not filter out an unresolved problem because it began before the selected incident window. |
| Recent errors, Changes, Diagnostics, check runs | Evidence observed during the selected half-open window `[from, to)`. | Retained query/history APIs and their availability metadata. Log counts are sampled occurrences, not request error rates. |
| Finding history | Episodes overlapping the period, with observed transitions shown separately. | A recovered episode is not currently active; a missing recovery does not prove continued failure across a history gap. |
| Resource detail | Current identity/state header; selected-period evidence inside historical tabs. | Historical failures retain their original identity and values. Do not present today's configuration as configuration at incident time. |
| Costs | Charge-day trend or explicitly selected invoice period, independent of health time. | Published billing revisions, basis, currency, and source status. Daily costs cannot explain minute-level incident changes. |

Define a typed route-state contract for scope, tab, search, sort, and absolute or relative time. Use one normalization path for URL state and backend filters. Existing `service` query values describe observed provider/service metadata; add a distinct application-service selector rather than changing that meaning. Current dashboard endpoints accept fewer filters than retained query endpoints, so add server-side projections before exposing unsupported controls. Reject conflicting project/account/subscription aliases; scope filters select evidence within the shared reader boundary and are not tenant authorization.

Pause freezes the loaded view and resolves relative time to absolute bounds; it does not create a durable snapshot of the fleet. Historical rows may arrive late or be corrected, even with fixed time bounds. Explicit refresh can incorporate them with an updated capture time. Do not promise repeatable historical results across new requests until a revision contract supports them. Keep source timestamps separate from browser receipt time, and preserve gaps returned by the API. A pending deployment assessment shows its next useful observation time; it does not change collector cadence or bypass rollout grace.
...
Treat a wrong-account console session as a separate failure from an incorrect URL. AWS account and Azure tenant context must be displayed alongside region/subscription; a console URL does not reliably switch the user's authenticated account or role. Use only verified account-switch destinations and otherwise provide a copyable account identifier. Dashboard access grants no provider permissions. Distinguish “No retained logs in this window” from a broken destination or denied log access. Log links use the failure's source timestamps, not the time the button was clicked.
...
The current `Bus` retains one immutable view; current-list cursors contain ordering keys, while temporal cursors bind filters and a time window. Neither is a retained database snapshot. Add a process epoch to any externally compared generation so restart cannot reuse an old revision identity. Produce summary and its initial problem preview from one captured view; do not reconstruct overview totals from browser pages. Bind new list cursors to normalized filters and sort order. On a revision change during paging, retain the displayed rows with an update notice and offer a bounded refresh from the anchor; do not repeatedly chase the newest generation or mix totals while claiming a coherent snapshot. Any retained generations need an explicit global byte/count/age bound, never one fleet copy per browser.

Use server-side problem grouping and totals across the selected dataset, with paged group members; grouping the loaded 150 rows cannot produce fleet counts. Preserve existing ungrouped API and MCP behavior. Expired cursors, evicted history, changed filters, and late records need distinct tested restart behavior. On authentication failure, clear protected data and offer sign-in; on a transient collection/API failure, retain same-scope evidence with its age. Neither path should cause repeated automatic login or fetch loops.
...
The implementation checklist needs one row per service family with native operation, resource identifier, metric namespace/dimensions/statistic/unit, threshold or expected-state rule, required permission, fixture, and live result. Record gaps as absent implementation, incomplete behavior, or unverified behavior. In particular, EKS/AKS control-plane inventory does not establish Kubernetes workload access: verify private network reachability and read-only cluster authorization separately. Metric availability is not an evaluation rule; business-dependent thresholds remain unconfigured until specified. Keep API versions and partition/global/regional routing in adapters, with tests for unavailable regions and refresh after credential expiry.
...
Retain charge period and invoice/billing period separately. The daily trend uses provider charge timing normalized to UTC days; invoice reconciliation uses the provider billing period, which need not equal the sum of usage dates in that calendar month. Preserve the source timezone and conversion rule. Multi-day or period-only charges require an explicit allocation method or a separate period adjustment; never invent daily precision. Apply credits exactly once, including nested GCP credits and already-net FOCUS amounts. Define each adapter's source version and mapping before ingestion, using the [FOCUS billed-cost definition](https://focus.finops.org/docs/specification/v1-0/columns/cost-and-usage/billed-cost/) and [charge versus billing period distinction](https://focus.finops.org/docs/specification/v1-4/features/effective-cost-analysis/).

Compare matching source sets, currency, basis, and equally elapsed periods. Mark the current day provisional; disable percentage change when the comparison is missing or its denominator is zero or negative, and show the absolute difference. Import completion, provider period finality, reconciliation, and resource attribution are separate states. A fully imported export may still be provisional; unallocated cost can still reconcile. Keep maximum charge timestamp, export creation time, and last successful import separate; none alone proves coverage through that date.
...
Persist source object versions/ETags, manifest identity, query-job identity, adapter version, and published partition revision. Stage all parts, validate schema and totals, then atomically publish the replacement and checkpoint. A missing part, malformed record, changed object, lost database connection, or crash must retain the last successful revision with an explicit import fault. Resume an existing provider query job after timeout rather than submitting the same paid query again. Periodically reconcile closed periods within retention so late adjustments are not missed by an open-period-only scan. Reject duplicate/overlapping source coverage before summing; switching export formats replaces the source for a declared period instead of adding both.

Restrict export reads to configured accounts, buckets/containers, prefixes, and fixed API origins. Treat manifest object paths and provider records as untrusted input; reject path escapes, arbitrary URLs, oversized decompression, unsupported schema changes, and unapproved tags. Use fixed parameterized read queries, never caller-supplied SQL. Billing export setup belongs in devops; dashboard requests only read local aggregates and never start provider billing jobs.
...
Add read endpoints named `summary`, `series`, `breakdown`, and `sources` under `/api/v1/query/costs/`, using the existing query/error/auth conventions. A cost-specific date filter must support the proposed 13-month retention without widening the existing 31-day health query limit. Responses include requested/available period, charge versus invoice time basis, measure, currency, source freshness, completeness, grouping, and publication revision. Summary, chart, and breakdown use the same bounded revision selection; bind cursors to filters, sort, currency, measure, and revision, and return an explicit refresh-required error when a revision expires. Aggregate charts server-side at a resolution appropriate to the range; paginate breakdown rows and expose Other without losing totals. Retained temporal health APIs supply investigation windows; current-state APIs must not pretend to support historical reconstruction they do not have.

Keep Google IAP on every route and asset. The current authorization guard returns a boolean shared-reader decision; there is no application administrator role. Until billing readership is decided, keep cost endpoints disabled and source configuration in devops. If the existing reader group is approved, reuse that boundary. If narrower access is needed, first retain the verified subject in request context and implement an explicit backend cost-read capability with a reviewed membership source and bounded refresh/revocation behavior. Do not assume ordinary Google IAP assertions include Workspace group membership; the documented identity claims are [subject and email](https://docs.cloud.google.com/iap/docs/signed-headers-howto). Hiding a tab is insufficient. Apply the same authorization to totals, source names, resource cost tabs, error details, and future cost query tools. Do not expose billing-account names, negotiated rates, or secrets in public fixtures, logs, or unauthenticated errors. Existing API/MCP clients retain compatible behavior.

### Capacity and operating cost

Preserve the stated roughly US$100/month operating target for the monitor itself, including its VM, disk, PostgreSQL, load balancer, network, telemetry, export storage, and billing-query charges. This is separate from the fleet spend displayed by Costs. Record measured baseline and incremental estimates before enabling ingestion; no current estimate is asserted by this review. Enforce both per-query scan limits and a finite cumulative import allowance. BigQuery `LIMIT` does not bound scan charges; use partition pruning, dry-run estimates, and [`maximumBytesBilled`](https://docs.cloud.google.com/bigquery/docs/best-practices-costs). Reaching an import allowance leaves prior data labeled stale and reports the gap without affecting health collection.

Measure combined application/PostgreSQL/OS memory, CPU, query latency, journal backlog, WAL, indexes, staging space, and free disk on the existing 2 GiB VM. Throttle or pause imports before health latency or history delivery regresses. Define import-memory, temporary-disk, statement-timeout, connection, and shutdown bounds in configuration. Index actual filter/sort paths; avoid retaining every arbitrary tag combination as a new aggregate dimension. Test storage headroom during partition replacement and cleanup, not just steady-state row counts. Do not expand always-on infrastructure without showing why bounded ingestion on the existing VM is insufficient.

## Delivery order and acceptance

| Order | Deliverable | Completion evidence |
| --- | --- | --- |
| 1 | Use the experience specification and visual proposals to capture current admin tasks, failing console links, and refresh traces; finalize page content with representative redacted fixtures. | A new coworker can identify what a view represents and reach the main problem without interpreting collector terminology. Fixtures include healthy, degraded, stale, denied, partial, empty, and long-identifier cases. |
| 2 | Implement the complete screen overhaul: Overview, Problems, Resources, detail tabs, Checks, and billing-chart interactions against typed fixtures while billing ingestion proceeds. | From Overview, locate what/where/when and open evidence in at most two navigation steps. Checks remain inspectable. Back/forward restores filters and position. Chart clicks filter breakdowns. Fixture verification is an intermediate milestone; real billing remains required. |
...
Orders 2 and 3 should ship before waiting for billing exports or cloud access. GCP billing ingestion can proceed alongside the UX overhaul; it does not wait for AWS/Azure monitoring completion. Design and test the visual billing experience early, then connect each real source independently. Scope and cost authorization decisions should be settled while those changes are underway. Build API/schema changes before dependent UI changes, and use additive migrations and compatible routes for rollback. Refresh frontend design conventions in `docs/design/fe/dashboard.md` and backend conventions in `docs/architecture/be/design.md` when the implementation is accepted.

Keep application/API changes in this repository and identity, export, configuration, service, and rollout changes in devops. Preserve a deployable UI/link change without cost migrations. Enable each new provider target and billing source separately after validation; switching off a failed import must preserve its last published revision. Rehearse binary rollback with additive tables still present, interrupted import recovery, and startup on an existing evidence/database directory. `/healthz` must remain available during slow collection, unavailable billing, or history faults; it is not a fleet-health verdict.

Browser acceptance includes Firefox, Chromium, and WebKit; keyboard-only navigation; light/dark modes; 200% zoom; desktop and narrow widths; retained scroll across paging; deep links; and unavailable/reconnecting states. Measure interaction latency and layout shifts under a representative large fleet. Initial targets: local input feedback under 100 ms, warm paged reads under 500 ms p95 on the deployed VM, and memory/task/connection counts that settle within configured bounds after warmup during a one-hour churn/import test. Record peak memory and disk as well as steady-state use; legitimate backfill growth must converge under retention. These are acceptance targets to measure, not current performance claims.
...
Add explicit regressions for a 15-minute view with a day-old unresolved finding; late evidence inside a paused window; pod replacement with the same name; severity changes across page boundaries; process restart resetting generation; scope change during a slow response; and loss of authorization during a refresh. Billing fixtures must cover all parts of a revised manifest, credit duplication, mixed invoice/charge months, nullable dimension uniqueness, source overlap, decimal overflow, zero/negative comparison totals, cross-month timezones, disabled cost access, and rollback after staging. Validate at least two unfamiliar coworkers' core tasks using redacted fixtures, and retain authenticated destination checks separately from automated URL tests.
