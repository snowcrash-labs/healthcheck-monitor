...
The subsequent billing incident exposed a contract gap: PostgreSQL and Rust accepted 38 fractional decimal places while the frontend still allowed 18. A valid billing response therefore rejected the entire page. Revision `304bda3` corrects frontend validation and integer arithmetic and was deployed on 2026-09-08. Verification matched the protected deployed JavaScript checksum and rendered a fresh production response in Firefox, Chromium, and WebKit. Future contract and deployment gates belong in the billing plan.
