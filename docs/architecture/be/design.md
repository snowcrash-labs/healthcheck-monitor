...
The `healthcheck-connect` executable serves stdio MCP to local clients and calls the existing HTTPS API through Google IAP. Its dependency graph excludes provider collectors and PostgreSQL. Google desktop authorization uses PKCE, a loopback callback, verified ID tokens, and OS credential storage. Token refresh is serialized and never initiated interactively by a tool call. The server continues to verify IAP assertions, trusted peers, audience, and identity domain on every API request. No remote MCP OAuth gateway or public authentication bypass is introduced.
