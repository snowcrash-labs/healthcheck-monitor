...
The HTTP boundary permits GET/HEAD metadata reads and a closed set of documented read-only POST operations. All configured AWS collection uses official SDK clients over a bounded reqwest transport, including the current CloudWatch RPC/CBOR protocol. Client caches include region, service, retries, and deadlines; compatible connections share a credential-free pool across targets. GCP and Azure management reads retain bounded REST adapters. Credential acquisition has separate, narrow exceptions for STS and local workload metadata. Endpoint probes inspect the HTTP connection's trusted certificate and discard response bodies. Provider payloads are projected before entering evidence or the inventory cache.

Remote operations share global and provider-scope admission limits independently of scheduler task limits. A busy scope waits before acquiring global capacity; reload keeps existing reservations charged. Independent Kubernetes kinds, KEDA metric reads, endpoint probes, and cloud inventory branches run concurrently with bounded fan-out. Continuation pages remain sequential. Concurrent inventory branches charge a shared cumulative memory budget before retaining evidence or follow-up descriptors. Queue ownership is indexed once per collection. HTTP clients share a bounded DNS cache; per-request deadlines and response limits come from the executing check. Kubernetes uses native raw send so failed responses cannot bypass response bounds inside the SDK's error parser.
...
Monitoring query contracts live in the lightweight `monitor-query` crate. The server projects allowlisted findings, log windows, check summaries, and observed releases from the same published state as the dashboard. A bounded journal writes indexed temporal records with PostgreSQL-generated UUIDv7 keys; unchanged findings and release metadata reuse intervals. Native transition history without rich diagnostics remains explicitly incomplete. Query filters preserve historical scope, source timestamps, and collection gaps. Historical read connections are separate from the journal connection, and query requests have a twenty-second deadline.

The `healthcheck-connect` executable serves stdio MCP to local clients and calls the existing HTTPS API through Google IAP. Its dependency graph excludes provider collectors and PostgreSQL. Google desktop authorization uses PKCE, a loopback callback, verified ID tokens, and OS credential storage or explicitly selected atomic owner-only files for headless sessions. Storage-access failures remain distinct from missing Google authorization; the connector never changes stores automatically. Token refresh is serialized and never initiated interactively by a tool call. The server continues to verify IAP assertions, trusted peers, audience, and identity domain on every API request. No remote MCP OAuth gateway or public authentication bypass is introduced.

Billing uses a separate bounded worker and PostgreSQL connections. Source-side aggregates are staged under database-generated UUIDv7 imports; a short transaction publishes all affected day mappings. Exact numeric amounts cross JSON as validated decimal strings, and absent dimensions remain nullable. Published rows are derived projections of provider-owned billing data. Keyset cursors bind the query and publication; a concurrent publication requests a refresh rather than mixing revisions. Native PostgreSQL advisory locking serializes embedded migrations. See the [billing runbook](../../plans/2026-09-07-dashboard-billing-operations.md) for accounting boundaries and rollback.
